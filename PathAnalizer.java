...
implements ActionListener, WindowListener
{
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if(e.getActionCommand().equals("Help"))
		{
			...
			helpFrame.setSize(425,800);
			JLabel helpLabel = new JLabel();
			helpLabel.setText("<html><p style=\"width:300px\">"+"The input consists of multiple occurences of the following: activity name, duration, and dependencies (predecessors).<br><br>There is no maximum on the number of activities and predecessors. <br><br>Activity names can be multiple characters. <br><br>Duration must be an integer.<br><br>If another user input is found, then an error is displayed, and the user must re-f another input before proceeding. The starting node or nodes do not have predecessors. Once all inputs are completed, then the processing can begin.<br><br>Output:<br>Pressing the enter button enters the information that is in the input fields into a hidden list that will be printed upon pressing the finish button. Input field is then cleared.<br><br>Pressing the finish button prints a list of all paths in the network that were submitted, with the duration of each path. The output field consists of the names of all activities in the path, displayed in descending order of duration. Additionally, there is a checkbox that allows the user to choose whether or not to display only the critical path(s) of the inputted nodes as opposed to all path(s).<br><br>About:<br>Prompts a new window that explains the project introduction and the overview of the program.<br><br>Restart:<br>Clears all fields of any inputs or outputs and restarts the process of inputting. Upon clicking, all inputs and paths must be re-entered and resubmitted.<br><br>Quit:<br>Halts any processes and quits out of the program immediately. No data will be saved.<br><br>Edit:<br>Allows the user to edit the duration of any previously entered nodes.<br><br>Generate Report:<br>Outputs the current time and date, all previously entered nodes, and the critical path(s) of all inputted nodes into a file name of the user's choice."+"</p></html>");
